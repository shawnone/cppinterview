...
//  cppinterview
//
//  Created by Shawnone on 13-5-26.
//
//
...
TEST(L127, normal)
{
	std::unordered_set<std::string> dict = {"hit","hot","dot","dog","lot","log","cog"};
	
	Solution127 solution;
	
	ASSERT_EQ(5, solution.ladderLength("hit", "cog", dict));
	
	dict = {"a", "b", "c"};
	ASSERT_EQ(2, solution.ladderLength("a", "c", dict));
	...
	ASSERT_EQ(12, solution.ladderLength("zings", "brown", dict));
...
TEST(L126, normal)
{
	...
	Print2LevelVector(solution.findLadders("hot", "dot", dict));
	
	dict = {"a", "b", "c"};
	Print2LevelVector(solution.findLadders("a", "c", dict));
